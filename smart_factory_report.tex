...
\begin{titlepage}
    ...
    \newline
    \textbf{Deployed Link:} \url{http://localhost:3001/}
...
\begin{itemize}
    ...
    \item \textbf{Database:} MySQL 8.0, Connection Pooling (mysql2)
    \item \textbf{Frontend:} Vanilla ES6+ JS, Forge CSS, Responsive HUD
    \item \textbf{Connectivity:} "Eternal Link" logic (Exponential Backoff + Visibility Sync)
    \item \textbf{Hardware:} ESP32 (simulated/physical) with C++ Ingestion Link
\end{itemize}
...
\begin{itemize}
    ...
    \item \textbf{Eternal Link Connectivity:} We implemented a robust "Eternal Link" system using exponential backoff (1s to 10s) and Visibility API listeners. This ensures the dashboard instantly reclaims its session the moment a user switches back to the tab, even on unstable industrial networks.
    \item \textbf{Active Pulse Pruning:} We implemented a server-side heartbeat mechanism to prune "zombie" connections, ensuring high availability and memory optimization during prolonged operation.
\end{itemize}
